...
:: 2. 检测 Rust 编译目标 (x86_64-pc-windows-gnu) 是否已安装
rustup target list --installed | findstr /c:"x86_64-pc-windows-gnu" >nul
if %errorlevel% neq 0 (
    echo [WARNING] 未检测到 x86_64-pc-windows-gnu 目标，正在安装...
    rustup target add x86_64-pc-windows-gnu
    if %errorlevel% neq 0 (
        echo [ERROR] 自动安装 target 失败，请手动运行: rustup target add x86_64-pc-windows-gnu
        exit /b 1
    )
)
...
copy /Y "target\x86_64-pc-windows-gnu\release\RustTLSProxy.dll" "RustTLSProxy.dll"
if %errorlevel% neq 0 (
